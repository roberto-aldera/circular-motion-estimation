...
# TODO -> stitch landmark sets together, ideally replacing 'bad' landmarks in one scan with some 'good' ones
# from surrounding scans

def stitch_landmarks(params):
    landmarks_path = params.landmarks_path
    figure_path = landmarks_path + "figs_stitched_landmarks/"
    ...
    plt.savefig("%s%s%s" % (output_path, "/landmarks_stitched", ".png"))
    plt.close()
...
def main():
    ...
    print("Running landmark stitching...")

    # get a landmark set in and plot it
    stitch_landmarks(params)
